...
public class LocateRegistry {
    ...
    public static Registry getRegistry(int port) {
        return getRegistry("127.0.0.1", port);
    }
    ...
    public static Registry createRegistry(int port) throws RemoteException {
        ...
        return (Registry) Proxy.newProxyInstance(Registry.class.getClassLoader(),
                new Class<?>[]{Registry.class}, new RegistryStubInvocationHandler("127.0.0.1", port));
    }


    public static Registry createRegistry(int port, String host) throws RemoteException {
        if (port <= 0) {
            port = Registry.REGISTRY_PORT;
        }
        if (host == null || host.length() == 0) {
            try {
                ...
                throw new RemoteException("Unable to determine local host address", e);
            }
        }
        Registry registry = new RegistryImpl(port, host);
